    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex h-16 items-center justify-between">
        <div class="flex items-center">
          <div class="shrink-0">
            <img class="size-15" src="/img/logo6.png" alt="Your Company">
          </div>
            ...
            <div class="relative ml-3">
              <div>
                ...
                <button type="button" @click="isOpen = !isOpen" class="relative flex max-w-xs items-center rounded-full bg-gray-800 text-sm focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 focus:outline-hidden" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                  ...
                  <img class="size-8 rounded-full" src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : asset('img/default-avatar.png') }}" alt="{{ Auth::user()->name }}">
                ...
                @else
                <a href="/login" class="text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md">Login</a>
                <span class="mx-2 text-gray-300">|</span>
                <a href="/register" class="text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md">Register</a>
                @endif
              </div>
              <div x-show="isOpen"
                ...
                <form action="/logout" method="POST">
                  @csrf
                <button type="submit" class="block px-4 py-2 text-sm text-gray-700 cursor-pointer" role="menuitem" tabindex="-1" id="user-menu-item-2">Logout</button>
                </form>
              </div>
            </div>
          </div>
    ...
    <div x-show="isOpen" class="md:hidden" id="mobile-menu">
      ...
      <div class="border-t border-gray-700 pt-4 pb-3">
        ...
        <div class="flex items-center px-5">
          <div class="shrink-0">
            <img class="size-10 rounded-full" src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : asset('img/default-avatar.png') }}" alt="{{ Auth::user()->name }}">
          </div>
          <div class="ml-3">
        ...
        <div class="mt-3 space-y-1 px-2">
          ...
          <form action="/logout" method="POST">
                  @csrf
                <button type="submit" class="text-start block w-full rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white cursor-pointer" role="menuitem" tabindex="-1" id="user-menu-item-2">Logout</button>
                </form>
        </div>
        @else
